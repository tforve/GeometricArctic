...
public class CharacterController2D : MonoBehaviour
{
	...
	//Particle
	//Jump
	[SerializeField] private ParticleSystem[] jumpParticleSpawn;	
	...
	private void FixedUpdate()
	{
		...
		for (int i = 0; i < colliders.Length; i++)
		{
			if (colliders[i].gameObject != gameObject)
			{
				...
				{
					//Particle System Spawner for Landing
					for (int j = 0; j < jumpParticleSpawn.Length; j++)
					{
						jumpParticleSpawn[j].transform.position = this.transform.position + (new Vector3(PS_spawnToFeetOffsetX, PS_spawnToFeetOffsetY, 0.0f));
						jumpParticleSpawn[j].Play();
					}
	...
	public void Move(float move, bool crouch, bool jump)
	{
		...
		if (m_Grounded || m_AirControl)
		{
			...
			//Spawning ParticleSystem to the feet when walking.
			SpawnParticle(move);
	...
	private void SpawnParticle(float move)
	{
		if (move > 0 && m_Grounded || move < 0 && m_Grounded)
		{
			//walkParticleSpawn.transform.position = this.transform.position + (new Vector3(0.0f, PS_spawnToFeetOffsetY, 0.0f));
